...
# `git diff` does not.
#
# `status-app.html` was missing from this list until an audit found it: it is
# regenerated by `--write` exactly like the other two and was added to the tool
...
TRIALS = [
    ...
    Trial(
        # The mirror of the trial above: a question an ADR announced it closed,
        # left sitting in the open list. Q36 did exactly this for four commits.
        "question-closed", "ADR-084 — a question an ADR closed is not still open",
        QUESTIONS,
        "## Needed at M2",
