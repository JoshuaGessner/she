...
| Levels | **What carries a party from one floor to the next?** `DES-015` is a three-floor expedition and nothing descends between them: `RunFile` holds no floor by design, and `GameState.descents` counts a lineage's descents rather than a run's. Until it exists the Delvings' depth comes from `--floor=N` and the Threshold still opens onto the Deep (ADR-183) | `DES-015`, `DES-005`, `TEC-003`, ADR-183 |
