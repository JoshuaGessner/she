...
| Movement | **Q111 — Keep the step-up nothing currently uses?** ADR-209 gave the player body a 0.40 m step after the walk panel stalled on what it measured as *0.30 m of stone on a 22° surface*; the developer approved it on the grounds that stepping small ledges does not break the movement's feel. ADR-213 then found the real fault — corridor ramps tilted along the diagonal of a turn — and after fixing it the step **fires zero times** on the panel: switched off, all nine floors still cross with identical leg counts. **Keep** it and it is insurance for geometry the panel does not contain (rubble, hand-built rooms, a future kit piece), at the cost of a movement rule no test currently exercises. **Remove** it and `DES-009`'s *"no parkour"* body is back to exactly what the design first described, which `CLAUDE.md`'s *prefer subtraction* favours. Whether a player ever notices is a feel question, and it is the developer's | `DES-009`, ADR-209, ADR-213 |
