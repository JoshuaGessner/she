...
| Q86 | How does the Ear behave for a **Vörðr** — a ghost makes no Clamor | `DES-019` | Core dark, ring live; scouting is their job |
| Q91 | Adopt FMOD from the start, or raw-Godot first and migrate? | `TEC-005` | Raw first — a week, no dependency risk, proves the state machine |
| Q92 | Confirm FMOD indie licensing terms at adoption | `TEC-005` | Terms change; verify before committing |
| Q93 | Do the three biomes share musical DNA, or are they separate voices? | `ART-003` | Composer's call — asked in the brief |
| Q88 | Does the Threshold theme grow with camp momentum? | `ART-002` | Yes if the stems exist — nearly free |
| Q90 | How does audio handle the Vörðr state? | `ART-002` | The world going quiet around you |
