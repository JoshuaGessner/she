...
## ADR-221 — A binding closes a wound, a hush rune swallows a room's noise, and something in the build is finally used

**Date:** 2026-09-13 · **Status:** accepted · **Closes `M4-T32`** · **Amends `DES-023` §3 and §4, `TEC-006`** · **Corrects ADR-220's note on floor 0**

**Context:** `M4-T17`'s report found that nothing heals during a run, and `components/health.gd` had documented a `heal()` since `M1` that was never written. Nothing in the build was *used*: everything was carried, worn, thrown or dropped. `DES-023` names two consumables for the slice — the linen binding and the hush rune — and ADR-217 recorded the developer's call for exactly those two.

### Decision

- **Things are used from the bag.** A `use_item` action (Q; pad Y), read only by `BagScreen` on the item under the cursor, and the bag shuts as you use it — linen is tied and a stave broken looking at the room, and the binding's ring is at the crosshair the open bag hides. **Y is shared with the class verb**, because the pad has no free button; the two are disjoint by the bag (`_hold` and `_snare` require `_bag <= 0.0`), recorded in `bind_gamepad.py`'s `SHARED_OK` beside `block`/`rotate_item`, which is the same argument.
- **Two traits, not one `ConsumableTrait` with an effect tag.** A tag is a string a system switches on while every effect's numbers share one resource, and a binding carrying a hush radius is a field nothing reads (ADR-064). `MendingTrait` has *seconds* and *restores*; `HushTrait` has *radius*, *seconds* and *crack*. `TEC-006`'s *"whether usable in combat"* is not built: the binding's rule for combat is that a blow breaks it, and nothing else would read a flag.
- **The binding: four seconds ⟨tune⟩ restoring 35% of maximum health ⟨tune⟩** — about one enemy blow on a bare body, so *a binding undoes a hit*. You may walk while tying it; **a blow breaks it, guarded or not, and so does a sprint**, going down, or spending a Waystone. It is **spent when the knot is tied**, the Waystone's order, so a broken binding costs the time and never the linen. It is refused on a body with no wound — charging the linen for a misclick at full health is a trap `DES-019` would answer with a confirmation, and it refuses those. **The bag asks the same rule before it sends anything** (`Player.can_use`, read by the bag and by the host), so a press that would do nothing leaves the bag open rather than shutting it on nothing — found reviewing the first version, which shut the bag and was then refused. The reference is *Escape from Tarkov*'s medical items: a timed use you can walk through and lose to a sprint. The sprint is judged as the host sees the body move, averaged over about a tenth of a second, because a client's body arrives twenty times a second and two packets landing in one frame read as a sprint for that frame.
- **The hush rune: broken at your feet, a circle of 6 m ⟨tune⟩ and one storey (3 m) ⟨tune⟩ where nothing sounds for 10 s ⟨tune⟩, then a crack of 10 clamor ⟨tune⟩ at its centre.** The circle does not silence anything itself: sound enters the world at `ClamorSource.add` and `ClamorField.deposit`, and both ask `Hush.silences` first, so no new sound can forget to check. A body standing inside also carries no radius (`audible_radius`), because a bag of coin rings whether or not you move. The crack is a `ClamorSource`, not a field deposit, so enemies' ears hear it as well as the Gullsjúkr. The reference is *Thief*'s moss arrow — silence laid on the floor, so the question is *where* — with the price moss never had. **Not one at a time**, unlike the snare: each rune is spent by breaking it. An inner ring closes on the centre as the time runs out and the crack flares the edge (`DES-018`). Live circles are a static list rather than a group, because every sound and every enemy's ears ask each frame, and a floor with no circle must pay nothing.
- **Each kit carries two bindings, through the stash.** `GameState.take_the_oath` already stashes a kit entry with no slot and the first descent carries the stash down (ADR-127), and `--class-probe` already counted it. `tests/data_probe.gd` failed every slotless kit entry as *"dropped on the floor of `_dress_the_body`"*, which was never true and went unread only because no kit had cargo; the rule is removed.
- **Bindings are filler from floor 1, not floor 0** — a correction to ADR-220's closing note, found measuring: dealt from floor 0 they were 1.7 per solo floor 0, five bindings in a solo run and 175% of a health bar, which is not *scarce*. **The rune is machine gear from floor 1**, as `DES-023` has it.
- **Found and fixed: the Waystone's ring was hidden unless something was in reach.** `Reticle._draw_channel` sat below the reach test's early return, so a Waystone spent in an empty corridor — the ordinary case — drew nothing, which is the fault the ring was built for. It draws first now, and the binding shares it.

### What changed on the floor, measured

Seeds 8000–8039 per depth, against ADR-220's table:

| | floor 0 | floor 1 | floor 2 |
|---|---|---|---|
| bindings dealt, solo | 0 | **0.68** a floor | **0.68** a floor |
| hush runes dealt | 0 | **4 floors in 40** | **3 floors in 40** |
| fixtures + solo filler | 22 → **22** | 174 → **144** | 294 → **270** |
| fixtures + party-of-four filler | 33 → **33** | 315 → **248** | 496 → **448** |

A solo run carries about **3.4 bindings** — two from the kit and one on roughly two floors in three below the top — or about 120% of a health bar across three floors ⟨tune⟩. The climb still holds. **Three things for a person's attention at `GATE M4 GREED`:** bindings take filler rooms, so floor 1 solo pays 17% less; the rune is rare enough that about one run in six finds one, which may be too rare to test; and **the seax has dropped out of machine gear**, because the rune sorts before it among worthless gear and machines deal from the top — a tie broken by id, which is deterministic and arbitrary.

### Verification

`--use-probe`, new and required by `check_scripts.sh` on its last row. Through the path a press takes — an item in the bag, `ask_to_use`, the host's countdown — it checks: `heal()` adds, stops at the maximum and refuses the dead; a binding is half-tied at two seconds, still in the bag, drawn at the crosshair with nothing in reach, then restores 35% and is spent; it is refused at full health and the bag would not send it, while it would send a rune; a blow at one second breaks it and keeps the linen; a sprint breaks it and **a walk does not**; inside a broken rune a step raises nothing, a body carries no radius and the field takes no deposit, while a deposit just outside lands; the rune leaves the bag; after ten seconds it cracks, is heard beyond the circle from an enemy's ear height, and a step inside sounds again. **Every row was planted and failed**: `heal()` without the dead check and without the cap, the binding spent when begun, the full-health refusal removed from the shared rule, a rune refused by it, the blow's break removed, the sprint's break removed, a walk judged at twice its speed, each of the three silencing checks removed, the circle's radius multiplied a hundredfold, the crack's pulse removed, the circle outliving its crack, and the reticle's old order restored.
