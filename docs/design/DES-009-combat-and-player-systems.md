...
- **Hazards are universal.** Traps, fire, water, and falls apply identically to enemies. Enemies of hostile factions fight each other.
  ...
  > **Built (ADR-236).** Laid only by a machine, over the whole of its room (`DES-015`). **Scree is loud whatever you do**, by the developer's call: every step on it is at least a set loudness — 4.0, above a sprint ⟨tune⟩ — crouched, with the Wing's silent crouch, or on an enemy's feet, which make no sound anywhere else. **Choke-damp** puts a lamp out and will not let it open, holds a player's stamina and an enemy's poise where they are, and no body calls the floor from inside it. In blockout, scree is a flat stone-coloured layer over the room's floor and the damp a translucent haze filling it to head height, so each can be seen from the doorway — the look is `M4-T05`'s to make good.
