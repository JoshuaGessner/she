...
>
> **The one special case is bought (ADR-249).** The Gullsjúkr has a diegetic sound of its own — its weight, never the reserved note — and when more than half of it is behind stone that sound is placed **in the doorway on the room-path between the ear and it**, with the detour spent as loudness. `FloorSource.way_of_sound` answers for that one source: the hand-built Deep reads its door table, a generated floor walks the mission graph and matches the corridor that serves both rooms. Nothing else calls it, which is what keeps this a special case rather than the general system this document refuses to buy.
>
> **The one special case is bought (ADR-249).** The Gullsjúkr has a diegetic sound of its own — its weight, never the reserved note — and when more than half of it is behind stone that sound is placed **in the doorway on the room-path between the ear and it**, with the detour spent as loudness. `FloorSource.way_of_sound` answers for that one source: the hand-built Deep reads its door table, a generated floor walks the mission graph and matches the corridor that serves both rooms. Nothing else calls it, which is what keeps this a special case rather than the general system this document refuses to buy.
>
