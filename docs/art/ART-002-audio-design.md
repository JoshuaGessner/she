...
> **BUILT AT `M4-T12` (ADR-247).** Occlusion is five rays from the ear to each source and a lerp of the per-source low-pass — gradient, so a doorframe is a sound half-hidden rather than a switch — staggered four sources a frame inside `TEC-005`'s budget. **Reverb is driven by the room the *listener* stands in**, not the source: one effect in line on the world's two buses, the level handing over how far across its room is. Outdoors returns nothing; her hall is the vast end of it. What remains of this task is the Hunter's own sound and the portals it comes through, which `TEC-005` scopes to it alone.
...
> **DECIDED (ADR-167, clarified by ADR-168):** **You keep the world and lose yourself.** To say the load-bearing half plainly: **the Vörðr hears everything, at full volume, exactly as the living do.** Nothing is muffled, ducked, filtered or removed from what reaches a dead player — the mix they hear is the mix they would have heard alive. What changes is only what they *emit*. The Vörðr makes no sound — no footsteps, no gear, no Clamor, because `M3-T14` already says it cannot be heard and a body that makes noise nobody can react to would be a lie in the mix. But the world does *not* go quiet: the party stays at full presence, and the Hunt stays exactly as loud as it is.
